...
function EditModal({ closeModal, editTask, dispatch }) {
    ...
    const [upadteTask, setUpadteTask] = useState(editTask.text)
    const [updateDueDate, setUpdateDueDate] = useState(editTask.dueDate || '');

    const updateHandeler = () => {
        if (upadteTask.trim() !== "") {
            dispatch({
              type: "edit",
              payload: { task: { ...editTask, text: upadteTask }, index: editTask.index }
            });
            closeModal()
         } else {
